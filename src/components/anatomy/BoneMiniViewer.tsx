/**
 * BoneMiniViewer — small 3D preview highlighting a single bone by TA98 ID
 */
...
function IsolatedSkeleton({ boneId }: { boneId: string }) {
  ...
  const preparedScene = useMemo(() => {
    ...
    const bonePos = bone ? new THREE.Vector3(...bone.position) : new THREE.Vector3(0, 1, 0);
    ...
    clone.traverse((child) => {
      ...
      mesh.material = new THREE.MeshStandardMaterial({
        ...
        transparent: !isTarget, opacity: isTarget ? 1.0 : 0.08,
        depthWrite: isTarget, side: THREE.DoubleSide,
      });
    });
